...
<html lang="en">
  <head>
  	<title>Data Referensi Style</title>
    <meta charset="utf-8">
  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5 pt-5 shifted">
        <div class="data">
          ...
          <h2 align="center">Data Referensi Style Modern</h2><br>

          <a href="{{ route('data_style.create') }}" class="btn btn-success">Tambah Data</a>
          ...
          <table class="table">
              ...
              <tbody>
                  ...
                  <tr align="center">
                      <td>{{ $item->jenis }}</td>
                      <td><img src="{!! asset('storage/app/'.$item->image) !!}" alt="$item->image" width="200" height="200"></td>
                      <td>
                        <div style="display: flex; gap: 10px;">
                            <a href="{{ route('data_style.edit', $item->id) }}" class="btn btn-primary">Edit</a>
                            <form action="{{ route('data_style.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
